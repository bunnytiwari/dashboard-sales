...
def authenticate_user():
    if "authenticated" not in st.session_state:
        ...
        st.text_input(label="password :",value="",type = "password",key="passwd",on_change=creds_entered)
        return False
    else:
...
if authenticate_user():
    ...
    if selected == "summary":
        st.title("CONCLUSION:")
        expander = st.expander("plot of sales by category")
        expander.write(
            "Discover the goldmine of profitability within the clothing and accessories category! With sales soaring, this vibrant sector promises lucrative returns and undeniable appeal."
        )
        expander = st.expander("plot of sales by Payment Method")
        expander.write(
            "People most preffered to pay by Credit Card."
        )
        expander = st.expander("plot of Purchase Amount (USD) by Season and Item Purchase")
        expander.write(
            "In the winter season, people make the biggest purchases"
        )
        expander = st.expander("purchase Amount by age")
        expander.write(
            "People over 40 tend to enjoy shopping more."
        )
